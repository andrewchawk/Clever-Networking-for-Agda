...
\begin{itemize}
	...
	\end{itemize}
	\item \AgdaField{ProtocolSet.universalSerialize} \AgdaBound{ps} is, as indicated by the name, a universal packet serialization function for protocols which explicitly do serialization.  \emph{Regardless of whether or not the \AgdaBound{p} protocol is actually allowed to carry the \AgdaBound{pPayload} protocol}, \AgdaField{ProtocolSet.universalSerialize} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaBound{p} is a serialized representation of the \AgdaBound{p} packet.
	\item \AgdaField{ProtocolSet.universalSerialize} \AgdaBound{ps} is, as \emph{this} name \emph{also} indicates, a universal packet \emph{de}serialization function for protocols which explicitly do serialization.  \emph{Regardless of whether or not the \AgdaBound{p} protocol is actually allowed to carry the \AgdaBound{pPayload} protocol}, if and only if \AgdaBound{n} can be interpreted as representing some sort of appropriate packet, then \AgdaField{ProtocolSet.universalSerialize} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaUnderscore{} \AgdaBound{n} is a representation of the \AgdaBound{n} packet.
...
\begin{code}
  record ProtocolSet (l : Level) : Set (Level.suc l) where
    field
      ...
      universalSerialize :
        (p pPayload : Protocol l) ->
        Protocol.isSerialized p ≅ true ->
        Protocol.isSerialized pPayload ≅ true ->
        UPacket pPayload ->
        Σ ℕ Fin
      ...
      deserialize-serialize-is-just :
        ...
        (spP : Protocol.isSerialized pPayload ≅ true) ->
        (packet : UPacket pPayload) ->
        just packet ≅
        universalDeserialize p pPayload sp spP
          (universalSerialize p pPayload sp spP packet)
\end{code}
